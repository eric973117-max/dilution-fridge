...
export function createPlateLabels(host) {
  ...
  function setActive(ids, nextSide, focusId) {
    ...
    focus = focusId ?? null;
    /* 立刻同步一次显隐：模块离开时间轴之后不会再调 update()，
       只记 active 的话名牌会留在屏幕上压住后面几层。 */
    nodes.forEach((rec, id) => {
      const on = active.includes(id);
      rec.el.classList.toggle('is-on', on);
      rec.el.classList.toggle('is-focus', on && focus === id);
      rec.el.classList.toggle('is-min', on && focus && focus !== id);
      if (!on) rec.el.style.opacity = '';
    });
  }
  ...
  function update(camera, plates, chambers = []) {
    ...
    [...plates, ...chambers].forEach((plate) => {
      ...
      /* 锚点两种写法都收：
         · { posWorld }            —— 世界坐标固定（腔体那类不会动的）
         · { part: {obj,offset} }  —— 挂在零件上（本轮新层的写法）
         · { part, offset }        —— 老写法（迁移过来的冰箱层） */
      const anchor = plate.part && plate.part.obj
        ? { obj: plate.part.obj, offset: plate.part.offset || plate.offset }
        : (plate.part && plate.offset ? { obj: plate.part, offset: plate.offset } : null);
      const world = plate.posWorld
        ? plate.posWorld.clone()
        : (anchor && anchor.offset ? anchor.obj.localToWorld(anchor.offset.clone()) : null);
      if (!world) {
        console.warn('[labels] 这个锚点既没有 posWorld 也没有可用的 part/offset：', plate.id);
    ...
    /* 下界要留在层级轨道之上（桌面轨道会折成两行，约占 80px） */
    const overflow = last ? box(last).bottom - (h - 150) : 0;
  ...
  /** 换语言 / 换数据后重建：清掉已挂的节点，下一帧 update 会按新数据重新挂 */
  function reset() {
    nodes.forEach((rec) => rec.el.remove());
    nodes.clear();
    active = [];
    focus = null;
  }

  return { setActive, update, reset };
}
