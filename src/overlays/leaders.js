/* ==========================================================================
   ...
   从「稀释制冷机-下午版」的 src/leaders.js 迁移。原来的分段数据写死在那台机器上，
   现在由模块通过 items 传进来：
     items = [{ id, accent, en, zh, temp, spec, anchor: [x,y,z], fallback?: [x,y,z] }]
   调用方（模块）每帧调 update(camera, machine, cardEl)，machine 需提供
   segments: Map<id, { group, anchorPart? }>。
   ========================================================================== */
...
export function createLeaders(host, { items: itemSpecs = [] } = {}) {
  ...
  itemSpecs.forEach((s) => {
    ...
    tSpec.textContent = `${s.temp || ''} · ${s.spec || ''}`;
  ...
  function pickAnchors(machine) {
    ...
    machine.segments.forEach((s, id) => {
      ...
      if (!best) {
        /* 没有挂到几何的段（零件默认隐藏之类）：用模块给的兜底锚点钉在机体上 */
        const fb = itemSpecs.find((s) => s.id === id)?.fallback;
  ...
  const ANNOT_W = 268;
  const ANNOT_H = 74;
  const ANNOT_TOP = 196;      // 让开顶部 HUD
  const ANNOT_BOTTOM = 152;   // 让开底部的层级轨道（新站轨道会折成两行）
  ...
  function updateAnnotated(camera, cardEl) {
    ...
    /* 右列不能让进章节卡那一栏（新站卡片固定在右侧），否则标注牌会压住正文 */
    const cardLeft = cardEl && cardEl.classList.contains('is-on')
      ? cardEl.getBoundingClientRect().left
      : W;
    const rightEdge = Math.max(colX + ANNOT_W + 24, Math.min(W - 24, cardLeft - 18));
    ['left', 'right'].forEach((side) => {
      ...
      list.forEach((it, i) => {
        ...
        layoutAnnot(it, right ? rightEdge - ANNOT_W : colX - ANNOT_W, y, right);
      });
    });
  }
  ...
  function update(camera, machine, cardEl) {
    if (annotated) { updateAnnotated(camera, cardEl); return; }
