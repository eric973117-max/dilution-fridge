...
/** 材质表。mono = true 时全部转成黑白（灰阶）：不保留任何色相，
    与稀释制冷机那一层的"白模 + 线稿"是同一套语言。 */
export function createMaterials({ mono = false } = {}) {
  const materials = {
  ...
  if (mono) {
    const gray = (hex) => {
      ...
      const l = Math.round((0.2126 * c.r + 0.7152 * c.g + 0.0722 * c.b) * 255);
      return new THREE.Color(`rgb(${l},${l},${l})`);
    };
    Object.values(materials).forEach((m) => {
      m.color.copy(gray(m.color.getHex()));
      if (m.emissive) { m.emissive.copy(gray(m.emissive.getHex())); m.emissiveIntensity = Math.min(m.emissiveIntensity ?? 0, .22); }
      m.metalness = Math.min(m.metalness ?? .3, .45);
    });
  }
