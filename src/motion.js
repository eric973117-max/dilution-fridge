...
/* 手机端「时间轴拉长」系数（只在 ≤900px 生效，宽屏恒为 1）。
   ...
   所以把每一章的滚动行程拉长；但拉太长又变成"划不动"，用户体验下来
   1.6 偏慢、1.0 偏快，取中间值：
     · 1.0（最早）  一张卡 ~727px 手指数 ≈ 2.3 次滑动  —— 太快，看不清
     · 1.6（上一版）一张卡 ~1282px ≈ 4 次滑动            —— 太慢，效果不好
     · 1.3（现在）  一张卡 ~1040px ≈ 3.3 次滑动          —— 折中
   桌面端恒为 1，一点不受影响。 */
const MOBILE_SCROLL_STRETCH = 1.3;
const scrollStretch = () =>
